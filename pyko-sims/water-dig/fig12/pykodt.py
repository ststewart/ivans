...
VERSIONS
    2024-05-12: v0.8.3 -- added dusty ideal gas model 'DIG'
    2024-02-22: v0.8.2 -- added eps1, eps2 to output
    2024-01-11: v0.8.1 -- moving outer boundary condition
    2023-11-10: v0.8 -- tweaks for vapor plume paper
    2023-07-05: v0.7 -- what difference?
...
#
# GLOBAL VARIABLE
__version__ = 'v0.8.3-dev-2024-05-12-dt' 
#__version__ = 'v0.8.2-dev-2024-02-22' 
#__version__ = 'v0.8.1-dev-2024-01-11' 
#__version__ = 'v0.8-dev-2023-11-10' 
#
...
class RunClass:
    ...
    def __init__(self,fin='config.yml',fout='output.dat',ftype='YAML'):
        ...
        self.dtmin     = 1.e-10  # DEBUG later add a check for too small time steps to halt the calculation
    ...
    def binaryoutput(self):
        ...
        with open(self.outputfilename+'.parameters',"wb") as f: 
            print('dumping dataout class with pickle ',self.stepn,self.time)
            pickle.dump(self,f)
    def checkinput(self):
...
class IDGClass:
    ...
    def __str__(self):
        ...
        #
class DIGClass:
    """ Dusty Ideal Gas EOS: Material Parameters 
        and initial state of the homogeneous material layer """
    def __init__(self):
        self.name   = ''  # name of the material, string 
        self.gamma0 = 0.  # Thermodynamic grueneisen parameter, dimless 
        self.cv     = 0.  # Heat capacity at constant volume, (Mbar cm3)/(K g)
        self.Z      = 0.  # Volumetric dust fraction, dimless
        self.kdust  = 0.  # Dust mass to total mass fraction, dimless
        self.rhod   = 0.  # dust density g/cm3
        self.rhog   = 0.  # gas density g/cm3
        self.cvd    = 0.  # dust specific heat capacity (Mbar cm3)/(K g)
        self.cvg    = 0.  # gas specific heat capacity (Mbar cm3)/(K g)
        self.rho0   = 0.  # initial state density, g/cm3
        self.p0     = 0.  # initial state pressure, Mbar
        self.up0    = 0.  # initial state particle velocity, cm/us=10 km/s
        self.iev0   = 0.  # initial state internal energy per original volume, 1e12 ergs/cm3 = 100 GJ/m3
    ...
    def __str__(self):
        """ Print the Ideal Gas material parameters """
        return f'\nClass Dusty Ideal Gas [code units]: {self.name} \n' + \
               f'   gamma0: {self.gamma0} \n' + \
               f'   cv: {self.cv} \n' + \
               f'   Z:  {self.Z} \n' + \
                '   initial state: \n' + \
...
class OutputClass:
    ...
    def __init__(self):
        ...
        self.dr       = np.zeros(0) # length of cell
...
class DomainClass:
    ...
    def makegrid(self,run,verbose=False):
        ...
        if len(iuppos0) == 0:
            ...
            # changed from 10 to 100 v0.8.1
            run.dtstart = np.min(dt0)/100.
        else:
...
#            self.pres[n+1,obcj] = self.pres[n,obcj-1]  # DEBUG sts added in v0.4 to test symmetry boundary 
            # sts test v0.8.1 moving outer boundary condition
            self.up[n+1,obcj] =  run.ibc[run.nbc-1].upbc #-0.3
            self.pres[n+1,obcj] =  run.ibc[run.nbc-1].pbc # self.pres[n,obcj-1] 
        if run.ibcid[run.nbc-1] == 'FREE':
            # free boundary ibc=1
            oghost_p = -run.ibc[run.nbc-1].pbc #for BORG input
        ...
        for imat in np.arange(run.nmat):
            ...
            if (run.istrid[imat] == 'VM'):
                ...
                if len(iyld)>0:
                    ...
                    #
            if (run.istrid[imat] == 'SG'):
                ### Wilkins Steinberg-Guinan strength model guided by the fortran KO v13 implementation by Borg but using the original temperature formulation in Wilkins
                ...
                for isg in ioddimat:
                    ...
                    if (k > 0.):
                        ...
                        # update the total plastic strain at n+2 by adding incremental plastic strain at n+2
                        # only add positive accumulation of plastic strain
                        if ((1./sscale)*eqsn2/(3.0*self.mu[isg]))>0.:
                            self.epstrain[n+2,isg] = self.epstrain[n,isg] + (1./sscale)*eqsn2/(3.0*self.mu[isg])
                        else:
                            self.epstrain[n+2,isg] = self.epstrain[n,isg]
                        # update accumulated elastic strain
                        # only add position accumulation of elastic strain???? it can be recovered.....
                        if (eqsn2-eqsn)/(3.0*self.mu[isg])>0.:
                            self.eestrain[n+2,isg] = self.eestrain[n,isg] + (eqsn2-eqsn)/(3.0*self.mu[isg])
                        else:
                            self.eestrain[n+2,isg] = self.eestrain[n,isg]
        #
...
#                self.alocal[ioddimat[icheck]] = sescs[icheck]\
        ...
        for imat in np.arange(run.nmat):
            ...
            if (run.ieosid[imat] == 'IDG'):
                ...
                self.temp[n+1,ioddimat] = (self.temp[n,ioddimat]+self.temp[n+2,ioddimat])/2.
            ########### DUSTY IDEAL GAS EOS #################
            if (run.ieosid[imat] == 'DIG'):
                # use Dusty Ideal Gas EOS
                # same as IDG except for (1-Z) term in the energy equation
                # gamma0 is the mixture gamma0
                # fill in staggered mesh with s1[n+1] stress deviators
                # doing it here after any yielding calculation
                # keep this until sure not needed for mixed material boundaries
                self.s1[n+1,ioddimat] = 0.5*(self.s1[n,ioddimat]+self.s1[n+2,ioddimat])
                ...
                #x = 1.-self.vr[n+2,ioddimat]
                mga = 0.
                mgb = (run.ieos[imat].gamma0-1.)/self.vr[n+2,ioddimat]/(1.-run.ieos[imat].Z)
                # calculate total internal energy E
                # uses current time step pressure P[n]
                self.iev0[n+2,ioddimat]=\
                    ( (self.iev0[n,ioddimat]- \
        ...
        for imat in np.arange(run.nmat):
            ...
            if (run.ieosid[imat] == 'SES'):
                ...
                #self.alocal[ioddimat[icheck]] = sescs[icheck]
                self.alocal[ioddimat] = sescs
                #mincs = 0.0001 # code units 10 km/s
                #icheck = npwhere(self.alocal[ioddimat] < mincs)[0]
                #self.alocal[ioddimat[icheck]] = mincs
            ########### Tillotson EOS #################
            if (run.ieosid[imat] == 'TIL'):
                for iii in ioddimat:
            ...
            ########### DUSTY IDEAL GAS EOS #################
            #if (run.ieosid[imat] == 'DIG'):
                # do nothing for now; the ideal gas equation should be the sound speed
                # sqrt(P/rho)
        a = self.alocal[ioddj]
        # ----------------------------- END SOUND SPEED CALC ------------------
        ...
        if 1:
            ...
            if np.min(deltat_tmp) > 1.1*dthalf:
                ...
                #v0.8.1
                ind_min_dt = np.where(deltat_next == deltat_tmp)[0]
        if self.time[n] < .5e6:
            deltat_next = 1. # test code with constant time step
            ind_min_dt = np.asarray([0])
...
#                if self.time[n+2] >= run.next_time_dump:
        ...
        if verbose: 
            if np.mod(self.stepn, 1000) == 0:
                if len(ind_min_dt)>0:
                    print("Count, time[n] sec, dt sec, index = ",self.stepn,self.time[n]/1.e6,self.deltat/1.e6, ind_min_dt)
                else:
                    print("Count, time[n] sec, dt sec, index = ",self.stepn,self.time[n]/1.e6,self.deltat/1.e6, ' dt fixed')
        ...
        #print('TEST dt_next=',self.deltat_next)
        #
    ...
    def createinteriorfracture(self,run,imat,ispall,ninteriorbc):
        ...
        return
    def writefirstoutput(self,run,checkexistsflag=True,verbose=False,overwriteflag=True):
    ...
    def binaryoutputpint(self,run,verbose=False):
        ...
        drarr = Q_(np.asarray(self.pos[n+2,ioddj+1]-self.pos[n+2,ioddj-1]),config['codeunits']['length'])
        dataout.dr    = drarr.to(config['units']['length'])
        eps1arr = Q_(np.asarray(self.eps1[n+1,ioddj]),config['codeunits']['strain_rate'])
        dataout.eps1  = eps1arr.to(config['units']['strain_rate'])
        eps2arr = Q_(np.asarray(self.eps2[n+1,ioddj]),config['codeunits']['strain_rate'])
        dataout.eps2  = eps2arr.to(config['units']['strain_rate'])
        if 0:
        ...
        run.outputmvtot  = np.append(run.outputmvtot,[self.mvtotal])
        #
    def onebinaryoutputpint(self,run,verbose=False,filetag='-one'):
        """ Create time snapshot of cell centered values ready for pandas dataframe. 
            The output class is in the units defined by the user input in the yaml configuration file.
            ONE TIME STEP ONLY - FILENAME HAS filtag added to end
        """
        # set up special code units for pint
        ureg.define('eu = 1.0E12 ergs')   # energy unit
        #
        ...
        with open(run.outputfilename+filetag+'.one',"wb") as f: 
            print('Writing single output file = ',run.outputfilename+filetag+'.one')
            #print('dumping dataout class with pickle ',self.stepn)
            pickle.dump(dataout,f)
        # update output log in the run object - do not do - this is a one-off time step save
        #run.outputsteps = np.append(run.outputsteps,[self.stepn])
        #run.outputtimes = np.append(run.outputtimes,[self.time[n+2]])
        # update the conservation values in the run object
        #run.outputietot  = np.append(run.outputietot,[self.ietotal])
...
#    Q_ = ureg.Quantity
    ...
    for imat in range(run.nmat):
        ...
        if config[matlist[imat]]['eos']['type'] == 'DIG':
            run.ieosid.append('DIG')
            matnew = DIGClass()
            run.ieos.append(matnew)
            run.ieos[imat].name   = config[matlist[imat]]['eos']['name'] # informal name
            # these variables extracted above
            ...
            run.ieos[imat].iev0   = iev0.magnitude
            run.ieos[imat].gamma0 = config[matlist[imat]]['eos']['gamma']
            # dusty gas parameters
            rhod = Q_(np.asarray(config[matlist[imat]]['eos']['rhod'],dtype='float'),config['units']['density'])
            run.ieos[imat].rhod = rhod.magnitude
            rhog = Q_(np.asarray(config[matlist[imat]]['eos']['rhog'],dtype='float'),config['units']['density'])
            run.ieos[imat].rhog = rhog.magnitude            # specific heat capacity cv
            spcvd = Q_(np.asarray(config[matlist[imat]]['eos']['cvd'],dtype='float'),config['units']['sp_heat_cap'])
            cvd   = spcvd.to(config['codeunits']['sp_heat_cap'])*rhod.to(config['codeunits']['density'])
            run.ieos[imat].cvd = cvd.magnitude
            spcvg = Q_(np.asarray(config[matlist[imat]]['eos']['cvg'],dtype='float'),config['units']['sp_heat_cap'])
            cvg   = spcvg.to(config['codeunits']['sp_heat_cap'])*rhog.to(config['codeunits']['density'])
            run.ieos[imat].cvg = cvg.magnitude
            rhod = Q_(np.asarray(config[matlist[imat]]['eos']['rhod'],dtype='float'),config['units']['density'])
            run.ieos[imat].Z = config[matlist[imat]]['eos']['Z'] # dimless
            run.ieos[imat].kdust = config[matlist[imat]]['eos']['kdust'] # dimless
            # convert input specific heat capacity to code units and then additional normalization by rho0 in code units
            # KO code units for cvv0 = eu/cm3/K
            spcv = Q_(np.asarray(config[matlist[imat]]['eos']['cv'],dtype='float'),config['units']['sp_heat_cap'])
            cv   = spcv.to(config['codeunits']['sp_heat_cap'])*rho0.to(config['codeunits']['density'])
        ...
        if config[matlist[imat]]['eos']['type'] == 'SES':
            ...
            #c0 = Q_(np.asarray(config[matlist[imat]]['init']['c0'],dtype='float'),config['units']['velocity'])
            #run.ieos[imat].c0  = c0.to(config['codeunits']['velocity']).magnitude
            #run.ieos[imat].gamma0 = config[matlist[imat]]['eos']['gamma0']
            # load the table
            run.ieos[imat].path   = config[matlist[imat]]['eos']['path']
            run.ieos[imat].EOS.R0REF = rho0.to(config['codeunits']['density']).magnitude
            #run.ieos[imat].EOS.CS0REF = c0.to(config['codeunits']['velocity']).magnitude
    ...
    if config['boundaries']['ibc'] == 'FREE':
        # free surface boundary condition
        run.ibc[ibc].name = 'Free surface '+str(ibc+1)
        if 'ibp0' in config['boundaries'].keys():
            pbc = Q_(np.asarray(config['boundaries']['ibp0'],dtype='float'),config['units']['pressure'])
        else:
    ...
    if config['boundaries']['obc'] == 'FREE':
        # free surface boundary condition
        run.ibc[ibc].name = 'Free surface '+str(ibc+1)
        if 'obp0' in config['boundaries'].keys():
            pbc = Q_(np.asarray(config['boundaries']['obp0'],dtype='float'),config['units']['pressure'])
        else:
            # if not entered by user, set free surface pressure to zero
            pbc = Q_(np.asarray(0.0,dtype='float'),config['codeunits']['pressure'])
        run.ibc[ibc].pbc = pbc.to(config['codeunits']['pressure']).magnitude
    if config['boundaries']['obc'] == 'FIXED':
        ...
        if 'obp0' in config['boundaries'].keys():
            ...
            pbc = Q_(np.asarray(0.0,dtype='float'),config['codeunits']['pressure'])
        run.ibc[ibc].pbc = pbc.to(config['codeunits']['pressure']).magnitude
        if 'obup' in config['boundaries'].keys():
            upbc = Q_(np.asarray(config['boundaries']['obup'],dtype='float'),config['units']['velocity'])
        else:
            # if not entered by user, set free surface pressure to zero
            upbc = Q_(np.asarray(0.0,dtype='float'),config['codeunits']['velocity'])
        run.ibc[ibc].upbc = upbc.to(config['codeunits']['velocity']).magnitude
    # -----------------------------------------------------------------------
    # stop time
...
#
@profile
def run(fin='pyko.yml',fout='pyko-output.dat',ftype='YAML',verbose=True,userdtstart=0.001,usertstepscale=0.,binoutput=True,debug=False,initarr=False):
    ############################################################## 
    ...
    if initarr != False:
        ...
        run.itempstart[0]=initarr[3]
        run.iiev0start[0]=initarr[4]
...
if __name__ == "__main__":
    ...
    #run(fin=filein,verbose=True)
    #
    # pass filein as command line arguments
    print(str(sys.argv[1]))
    run(fin=str(sys.argv[1]),verbose=True)
    #
#    
############################################################## 
############################################################## 
